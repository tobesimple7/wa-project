...
import {RowAlias} from "@/core/WaGrid.types"

export class WaGridRange {
    grid: WaGridCore;
    selector: string;
    ...
    removePanelRange = function (panelName = '') {
        ...
        if (panelName == 'panel30') {
            document.querySelectorAll(`${selector} .wa-grid-cell-start` ).forEach(cell => cell.classList.remove('wa-grid-cell-start'));
            document.querySelectorAll(`${selector} .wa-grid-panel31 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
            document.querySelectorAll(`${selector} .wa-grid-panel32 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
            document.querySelectorAll(`${selector} .wa-grid-panel30 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
        }
        else if (panelName == 'panel40') {
            document.querySelectorAll(`${selector} .wa-grid-panel41 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
            document.querySelectorAll(`${selector} .wa-grid-panel42 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
            document.querySelectorAll(`${selector} .wa-grid-panel40 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
        }
        else if (panelName == 'panel50') {
            document.querySelectorAll(`${selector} .wa-grid-panel51 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
            document.querySelectorAll(`${selector} .wa-grid-panel52 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
            document.querySelectorAll(`${selector} .wa-grid-panel50 .wa-grid-cell-select`).forEach(cell => cell.classList.remove('wa-grid-cell-select'));
        }
    }
    ...
    setRange = function (startRowIndex, lastRowIndex, startCellIndex, lastCellIndex, topRowIndex, type = 'add') {
        ...
        if (type == 'add' || grid.selectRangeArray == 0) {
            ...
            const selectRange: any = grid.selectRangeArray[0];
            selectRange.startRowIndex   = startRowIndex;
            selectRange.startCellIndex  = startCellIndex;
            selectRange.lastRowIndex    = lastRowIndex;
            selectRange.lastCellIndex   = lastCellIndex;
        ...
        if (grid.headerRowCount > 1) {
            const selectCell = function(trList, colIndex){
                for (let i = trList.length - 1; i >=0; i--) {
                    ...
                    else cell.classList.add('wa-grid-cell-select');
                }
            }
            // panel20
            let trList = document.querySelectorAll(selector + ' .wa-grid-panel20 .wa-grid-table tbody tr');
            for (let x = startCellIndex; x <= lastCellIndex; x++) selectCell(trList, x);
            // panel22
            if (grid.fixedColumnIndex != -1){
                trList = document.querySelectorAll(selector + ' .wa-grid-panel22 .wa-grid-table tbody tr');
                for (let x = startCellIndex; x <= lastCellIndex; x++) selectCell(trList, x);
            }
        }
        else {
            // panel20
            const tableCells = document.querySelectorAll(selector + ' .wa-grid-panel20 .wa-grid-table tbody tr:last-child td');
            if (tableCells.length == 0) return;
            for (let i = startCellIndex; i <= lastCellIndex; i++) {
                tableCells[i].classList.add('wa-grid-cell-select');
            }
        }
    }
    ...
    removeRange = function (startRowIndex, lastRowIndex, startCellIndex?, lastCellIndex?) {
        ...
        document.querySelectorAll(`${selector} .wa-grid-panel20 .wa-grid-table td`).forEach(function(td) {
            td.classList.remove('wa-grid-cell-select');
        });
        if (grid.fixedColumnIndex != -1){
            document.querySelectorAll(`${selector} .wa-grid-panel22 .wa-grid-table td`).forEach(function(td) {
                td.classList.remove('wa-grid-cell-select');
            });
        }
