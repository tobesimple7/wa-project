...
export class WaGridBaseLine {
    ...
    getFirstSelectedTableCell(this: WaGridCore, panelName: string) {
        ...
        const tableRows31 = document.querySelectorAll(selector + ' .wa-grid-panel31 .wa-grid-table tbody tr:not([style*="display: none"])');
        const tableRows = document.querySelectorAll(selector + ' .wa-grid-' + panelName + '  .wa-grid-table tbody tr:not([style*="display: none"])');
        ...
        Loop1: for (let i = 0, len = tableRows31.length; i < len; i++) {
            ...
            if (rowIndex == startRowIndex) {
                for (let x = 0; x < this.column_table.count(); x++) {
                    ...
                    if (tableCell.classList.contains('wa-grid-cell-start') || tableCell.classList.contains('wa-grid-cell-select')) {
                        startCell = tableCell;
                        break Loop1;
                    }
                }
    ...
    getLastSelectedTableCell(this: WaGridCore, panelName: string) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        let tableRows31= document.querySelectorAll(selector + ' .wa-grid-panel31 .wa-grid-table tbody tr:not([style*="display: none"])');
    ...
    clearSelectedLine(this: WaGridCore) {
    ...
    setSelectedLine(this: WaGridCore, lineWidth: number, lineHeight: number, rectTop: number, rectBottom: number, rectLeft: number, rectRight: number) {
    ...
    displaySelectedLine(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        if (grid.fixedColumnIndex != -1) {
            let tableRows30= document.querySelectorAll(selector + ' .wa-grid-panel30 .wa-grid-table tbody tr:not([style*="display: none"])');
            ...
            let rectMain   = document.querySelector(selector + ' .wa-grid-main').getBoundingClientRect();
            let rectTable31= document.querySelector(selector + ' .wa-grid-panel31 .wa-grid-table').getBoundingClientRect();
            let rectWrap   = document.querySelector(selector + ' .wa-grid-wrap').getBoundingClientRect();
            let rectPanel30= document.querySelector(selector + ' .wa-grid-panel30').getBoundingClientRect();
    ...
    displayOneSelection(this: WaGridCore, startRowIndex: number, startCellIndex: number) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        //============================================================= table display
        const trLeftList    	= document.querySelectorAll(selector + ' .wa-grid-panel31  .wa-grid-table tbody tr');
        const trContentList 	= document.querySelectorAll(selector + ' .wa-grid-panel30   .wa-grid-table tbody tr');
        const trFixBottomtList= document.querySelectorAll(selector + ' .wa-grid-panel32 .wa-grid-table tbody tr');

        count = topRowIndex + this.pageRowCount;
        //=============================================================
        const contentRect = document.querySelector(selector + ' .wa-grid-panel30').getBoundingClientRect();
        const contentTableRect = document.querySelector(selector + ' .wa-grid-panel30 > table').getBoundingClientRect();
        // @ts-ignore
        const contentTableLeft = parseInt(document.querySelector(selector + ' .wa-grid-panel30 > table').style.left, 10);
        ...
        for (let ri = topRowIndex; ri < count; ri++) {
            ...
            if (selectedValue == 1) leftTd.classList.add('wa-grid-cell-select');
            //============================================================= content
            for (let x = startColumnIndex; x <= lastColumnIndex; x++) {
                ...
                if (selectedValue == 1) {
                    if (this.startRowIndex == ri && this.startCellIndex == x) {
                        td.classList.add('wa-grid-cell-start');
                    }
                }
            }
            //============================================================= fixBottom
    ...
    getFirstDisplayRowIndex(this: WaGridCore, panelName = '') {
        let selector = '#' + this.gridId;
        const grid = this;

        if (this.view_table.count() == 0) return -1;

        const trList = document.querySelectorAll(selector + ' .wa-grid-panel31 .wa-grid-table tbody tr');
    ...
    getFirstRowIndex(this: WaGridCore, panelName = '') {
        // return : topRowIndex
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        let trList = document.querySelectorAll(selector + ' .wa-grid-panel31 .wa-grid-table tbody tr');
        // @ts-ignore
    ...
    getLastRowIndex(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;

        if (this.view_table.count() == 0) return -1;
        let trList = document.querySelectorAll(selector + ' .wa-grid-panel30 .wa-grid-table tbody tr:not([style*="display: none"])');
        let topRowIndex = this.getFirstRowIndex();
        return topRowIndex + trList.length - 1;
    }

    getLastTableRowIndex(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;

        const trList = document.querySelectorAll(selector + ' .wa-grid-panel30 .wa-grid-table tbody tr:not([style*="display: none"])');
