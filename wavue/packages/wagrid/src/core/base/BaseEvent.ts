...
export class WaBaseEvent {
    constructor() {

    }

    event_input(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;

        let panelInputt: any = document.querySelector(selector + ' .wa-grid-input-panel');
        ...
        const keyupEvent = function (e: any) {
            ...
            //e.stopPropagation();
            //e.stopImmediatePropagation();
            const input: any = document.querySelector(selector + ' .wa-grid-input');
            let colType = input.dataset.type;
        };
    ...
    event_input_icon(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        const mousedownEvent = function(e) {
            ...
            if (colType == 'date') {
                grid.waDate = new WaDate(grid, column, input);
            }
            else if (colType == CellType.combo) {
                grid.waCombo = new WaCombo(grid, column, input, input_code);
            }
        }
        input_icon.addEventListener('mousedown', mousedownEvent);
    }

    input_show(this: WaGridCore, e, mode) { //type : header, content, left, top
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        let input: any = document.querySelector(selector + ' .wa-grid-input');
        let input_code: any = document.querySelector(selector + ' .wa-grid-input-code');


        let panelMain: any =  document.querySelector(selector + ' .wa-grid-main');
        ...
        if (colType == 'date') {
            ...
            input_icon.childNodes[0].className = 'wa-grid-html-icon-combo';
        }
        else {
    ...
    input_hide(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;

        const panelInput: any = document.querySelector(selector + ' .wa-grid-input-panel');
        const input: any      = document.querySelector(selector + ' .wa-grid-input');
        const input_code: any = document.querySelector(selector + ' .wa-grid-input-code');

        // date, combo layer
        const input_icon: any = document.querySelector(selector + ' .wa-grid-input-panel-icon');
        //let inputLayerPanel= document.querySelector(selector + ' .wa-grid-input-layer-panel');
        ...
        if (this.waDate ) { this.waDate.destroy();  this.waDate = null; }
        if (this.waCombo) { this.waCombo.destroy(); this.waCombo = null; }
    }

    input_focus(this: WaGridCore) { if (this.isMobile == false) (document.querySelector('#' + this.gridId + ' .wa-grid-input') as any).focus(); }

    editStart(this: WaGridCore, e, mode) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        if (mode == 'key') {
            if (column[WaColumnProperty.editable] == true && grid.notNull(grid.onEdit)) {
                ...
                if (grid.isNull(result, true)) {
                    ...
                    // blur is certain
                    (document.querySelector(selector + ' .wa-grid-input') as any).blur();
                }
            }
        }
        else {
            if (column[WaColumnProperty.editable] == true && grid.notNull(grid.onEdit)) {
                ...
                if (grid.null(result) || result == true) {
                    ...
                    (document.querySelector(selector + ' .wa-grid-input') as any).blur(); // blur is certain
                }
            }
        }
    }

    editing(this: WaGridCore, e, mode) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    editEnd(this: WaGridCore, e?, mode?) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_addEventAll(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;

        this.waPanel20.panel21_select();
        this.waPanel20.panel20_select('panel20');
        this.waPanel20.panel20_select('panel22');

        this.waPanel30.panel30_select('panel30');
        this.waPanel30.panel31_select('panel31');
        this.waPanel30.panel30_select('panel32');

        this.waPanel40.panel40_select('panel40');
        this.waPanel40.panel41_select('panel41');
        this.waPanel40.panel40_select('panel42');

        this.waPanel40.panel40_select('panel50');
        this.waPanel40.panel41_select('panel51');
        this.waPanel40.panel40_select('panel52');

        this.waPanel70.panel70_select('panel70');
        this.waPanel70.panel70_select('panel72');
        // this.waPanel80.panel80_select();
        // this.waPanel90.panel90_select();
        ...
        document.addEventListener('scroll', function(e) {
            let panelInputList = document.querySelectorAll(selector + ' .wa-grid-input-panel');
            let inputList = document.querySelectorAll(selector + ' .wa-grid-input');
            let inputIconList = document.querySelectorAll(selector + ' .wa-grid-input-panel-icon');
            let inputPanelList = document.querySelectorAll(selector + ' .wa-grid-input-layer-panel');
        ...
        const bodyMouseDownEvent = function(e) {
            ...
            //
            // const inputPanel: any = document.querySelector(selector + ' .wa-grid-input-panel');
            // const inputLayerPanel: any = document.querySelector(selector + ' .wa-grid-input-layer-panel');
            //
            // if (   name.indexOf('wa-grid-panel10-filter-input') != -1
            //     || inputPanel.contains(e.target)
            //     || inputLayerPanel.contains(e.target)
            //     || name.indexOf('wa-grid-cell-filter-input' ) != -1
            //     || name.indexOf('wa-grid-cell-filter-combo' ) != -1) {
            // }
            // else {
            //     inputLayerPanel.style.left = '70000px';
            //     grid.input_focus();
    ...
    event_columnSort(this: WaGridCore, cell) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        //let sortDiv = cell.querySelector('.wa-grid-html-sort');
        let curSortKind = '';
        let sortKind = '';
    ...
    event_mobileTouchDrag(this: WaGridCore) { //type : header, content
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        let xWrap: any = document.querySelector(selector + ' .wa-grid-horizontal-scroll-wrap');
        let xBar: any = document.querySelector(selector + ' .wa-grid-horizontal-scroll-bar');
        let xPos: any = { left: 0, x : 0 }
        let yBar: any = document.querySelector(selector + ' .wa-grid-vertical-scroll-bar');
        ...
        const touchStartEvent = function(e) {
            ...
            document.querySelector(selector + ' .wa-grid-panel30').addEventListener('touchmove', touchMoveEvent);
            document.querySelector(selector + ' .wa-grid-panel30').addEventListener('touchend', touchEndEvent);
        }
        const touchMoveEvent = function(e) {
            ...
            if (Math.abs(xMove) > Math.abs(yMove)) {
                ...
                const header: any  = document.querySelector(selector + ' .wa-grid-panel20');
                ...
                setTimeout(function(){ grid.waPanel30.setDataPanel(topRowIndex);}, 5);
            }
        }
        const touchEndEvent = function(e) {
            ...
            if      (e.target.classList.contains('wa-grid-html-icon')) { targetName = 'icon'; tableCell = e.target.parentNode.parentNode; }
            else if (e.target.classList.contains('wa-grid-html-img'))  { targetName = 'img' ; tableCell = e.target.parentNode.parentNode; }
            else if (e.target.classList.contains('wa-grid-html-string')) { targetName = 'text'; tableCell = e.target.parentNode.parentNode; }
            else if (e.target.classList.contains('wa-grid-cell-div'))      { targetName = 'div' ; tableCell = e.target.parentNode; }
            else if (e.target.classList.contains('wa-grid-cell'))          { targetName = 'cell'; tableCell = e.target; }
            ...
            document.querySelector(selector + ' .wa-grid-panel30').removeEventListener('touchmove', touchMoveEvent);
            document.querySelector(selector + ' .wa-grid-panel30').removeEventListener('touchend', touchEndEvent);
            document.body.style.overflow = 'auto';
        }
        document.querySelector(selector + ' .wa-grid-panel30').addEventListener('touchstart', touchStartEvent, false);
    }

    event_columnResize(this: WaGridCore, panelName) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        const mouseDownEvent = function (e) {
            e.stopPropagation();
            if (e.target.classList.contains('wa-grid-html-resize')) {
            ...
            if (eventDetail == 1) {
                ...
                // @ts-ignore
                tableWidth = parseInt(document.querySelector(selector + ' .wa-grid-' + panelName + ' .wa-grid-table').getBoundingClientRect().width);

                resizer.classList.add('.wa-grid-html-resizing');
                ...
                if (tableCell.dataset.name == '') {
                    ...
                    let thCells = document.querySelectorAll(selector  + ' .wa-grid-' + panelName + ' .wa-grid-table thead th');
                ...
                e.stopPropagation();
                if (grid.isClassName(e.target, 'wa-grid-html-resize') != true) return;
        ...
        const mouseUpEvent = function (e) {
            if (eventDetail == 1){
                e.stopPropagation();
                resizer.classList.remove('.wa-grid-html-resizing');
                document.removeEventListener('mousemove', mouseMoveEvent);
                document.removeEventListener('mouseup', mouseUpEvent);
            }
        };

        const panel = document.querySelector(selector + ' .wa-grid-' + panelName);
    ...
    event_wheel(this: WaGridCore) { //mouse wheel event
        let selector = '#' + this.gridId;
        const grid = this;
        let xScroll: any = document.querySelector(selector + ' .wa-grid-horizontal-scroll');
        let yScroll: any = document.querySelector(selector + ' .wa-grid-vertical-scroll');

        const mouseWheelEvent = function (e) {
            let name = e.target.className;
            if (name.indexOf('wa-grid-input-combo') != -1) {
    ...
    event_scrollButton(this: WaGridCore) {
        //mouse wheel event
        //onmousedown -> onmouseup -> click
        let selector = '#' + this.gridId;
        const grid = this;

        let leftButton  = document.querySelector(selector + ' .wa-grid-horizontal-scroll .wa-grid-horizontal-scroll-left');
        let rightButton = document.querySelector(selector + ' .wa-grid-horizontal-scroll .wa-grid-horizontal-scroll-right');

        let upButton    = document.querySelector(selector + ' .wa-grid-vertical-scroll .wa-grid-vertical-scroll-up');
        let downButton  = document.querySelector(selector + ' .wa-grid-vertical-scroll .wa-grid-vertical-scroll-down');
        let flag = false;
        //-----------------------------------------------------------------
    ...
    event_railClick(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
        const xWrapClickEvent = function (e) {
            let bar: any = document.querySelector(selector + ' .wa-grid-horizontal-scroll-bar');
            ...
            if (e.target.className != 'wa-grid-horizontal-scroll-bar') {
                if (e.offsetX > left) {
        ...
        const yWrapClickEvent = function (e) {
            let bar: any = document.querySelector(selector + ' .wa-grid-vertical-scroll-bar');
            ...
            if (e.target.className != 'wa-grid-vertical-scroll-bar') {
                if (e.offsetY > top) {
            ...
            */
                ...
                bar = null;
                grid.waPanel30.setDataPanel(trTopIndex);
            }
        };
        //-----------------------------------------------------------------
        document.querySelector(selector + ' .wa-grid-horizontal-scroll .wa-grid-horizontal-scroll-wrap').addEventListener('click', xWrapClickEvent);
        document.querySelector(selector + ' .wa-grid-vertical-scroll   .wa-grid-vertical-scroll-wrap').addEventListener('click', yWrapClickEvent);
    }

    event_dragScrollBar(this: WaGridCore) {
        ...
        const xMouseDownEvent = function(e) {
            ...
            if (e.target.className != 'wa-grid-horizontal-scroll-bar') return;
            xPos.left = Number(xBar.style.left.replace('px', ''));
            xPos.x = e.clientX;
        ...
        const xMouseMoveEvent = function(e) {
            ...
            let header: any  = document.querySelector(selector + ' .wa-grid-panel20');
            let content: any = document.querySelector(selector + ' .wa-grid-panel30');
            let sum: any     = document.querySelector(selector + ' .wa-grid-panel40');
            let footer: any  = document.querySelector(selector + ' .wa-grid-panel50');
        ...
        //== y scroll
        const yBar = document.querySelector(selector + ' .wa-grid-vertical-scroll-bar');
        ...
        const yMouseDownEvent = function(e) {
            ...
            actveTopRowIndex = -1;
            if (e.target.className != 'wa-grid-vertical-scroll-bar') return;

            const yBar: any = document.querySelector(selector + ' .wa-grid-vertical-scroll-bar');
        ...
        const yMouseMoveEvent = function(e) {
            ...
            setTimeout(()=> { grid.waPanel30.setDataPanel(topRowIndex); }, 50);
        }
    ...
    tbs_moveCellLine(this: WaGridCore, cell, rowIndex, cellIndex) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        let table = document.querySelector(selector + ' .wa-grid-panel30 .wa-grid-table');
    ...
    isMovedPositionInConstRange(this: WaGridCore, startX, startY, lastX, lastY) {
    ...
    executeEvent(this: WaGridCore, eventType: string, param: any) {
        const grid = this;
    ...
    tbs_getMaxRowIndexByMouseMove(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_getMinRowIndexByMouseMove(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_getMaxCellIndexByMouseMove(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_getMinCellIndexByMouseMove(this: WaGridCore) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_getMaxRowIndexByMouseMove2(this: WaGridCore, panelName) {
        let selector = '#' + this.gridId;
        const grid = this;
        ...
        let tableRows= document.querySelectorAll(selector + ' .wa-grid-' + panelName + ' .wa-grid-table tbody tr:not([style*="display: none"])');
    ...
    tbs_getMinRowIndexByMouseMove2(this: WaGridCore, panelName) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_getMaxCellIndexByMouseMove2(this: WaGridCore, panelName) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_getMinCellIndexByMouseMove2(this: WaGridCore, panelName) {
        let selector = '#' + this.gridId;
        const grid = this;
    ...
    tbs_moveNextRowCell(this: WaGridCore, type) {
        ...
        let rowIndex = -1;
        let cellIndex = -1;
        let trList = document.querySelectorAll(selector + ' .wa-grid-panel30 .wa-grid-table tbody tr:not([style*="display:"])');
        let td: any = document.querySelector(selector + ' .wa-grid-cell-start');
        if (td == null) return;
        ...
        if (cellIndex == this.column_table.count() - 1 && dataRowIndex < this.view_table.count() - 1) {
            ...
            if (dataRowIndex == lastRowIndex - 1) this.waPanel30.setDataPanel(topRowIndex + 1);
    ...
    tbs_moveCell(this: WaGridCore, type) { //type : left, right, up, down
        let selector = '#' + this.gridId;
        const grid = this;

        let cellIndex = -1;

        let tableRows     = document.querySelectorAll(selector + ' .wa-grid-panel30 .wa-grid-table tbody tr:not([style*="display: none"])');
        let tableRowIndex, dataRowIndex;

        let tableCell: any = document.querySelector(selector + ' .wa-grid-cell-start');
        ...
        if (type == 'left') {
            ...
            let divContent = document.querySelector(selector + ' .wa-grid-panel30');
            let left2 = window.pageYOffset + divContent.getBoundingClientRect().left;

            if (left1 < left2) {
                let moveWidth= left2 - left1;
                let table20: any  = document.querySelector(selector + ' .wa-grid-panel20 .wa-grid-table');
                let table30: any  = document.querySelector(selector + ' .wa-grid-panel30 .wa-grid-table');
            ...
            let panel30= document.querySelector(selector + ' .wa-grid-panel30');
