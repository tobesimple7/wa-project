...
export class WaDataArrayTable extends WaBase {
    ...
    selectRowByRowId(arrayIndex: any, rowId: any) {
        let dataRows = this.selectRows(arrayIndex, WaColumnProperty.rowId, rowId, 1);
        return dataRows.length > 0 ? dataRows[0] : null;
    }

    selectRowIndexByRowId(arrayIndex: any, rowId: any) { return this.selectRowIndex(arrayIndex, WaColumnProperty.rowId, rowId); }
    ...
    removeByRowId(arryIndex: any, rowId: any) {
        let rowIndex = this.selectRowIndex(arryIndex, WaColumnProperty.rowId, rowId);
    ...
    update(arrayIndex: any, columnName: any, field: any, value: any) {
        let dataRows = this.selectRows(arrayIndex, WaColumnProperty.name, columnName);
    ...
    makeColIndex () : void {
        for (let i = 0; i < this.count(); i++) {
            ...
            columns.map((column: any, index: any) => column[WaColumnProperty.colIndex] = index);
        }
    }
}