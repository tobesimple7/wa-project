...
export class WaPanel30 extends WaPanelBase {
    ...
    createHtml(parentElement) {
        ...
        let s = '';
        s += '<div class="wa-grid-group31">';
            s += '<div class="wa-grid-panel">';
            s += '<div class="wa-grid-panel31"><table class="wa-grid-table"></table></div>';
            s += '<div class="wa-grid-panel32"><table class="wa-grid-table"></table></div>';
            s += '</div>';
        s += '</div>';
        s += '<div class="wa-grid-group30">';
            s += '<div class="wa-grid-panel">';
            s += '<div class="wa-grid-panel30"><table class="wa-grid-table"></table></div>';
            s += '</div>';
        s += '</div>';
    ...
    updateTableRows() {
        ...
        let tableRows =  document.querySelectorAll(selector + ' .wa-grid-panel30 tbody tr');
    ...
    setDataPanel(topRowIndex) {
        ...
        const tableRows =  document.querySelectorAll(`${selector} .wa-grid-panel30 tbody tr`);
        if (tableRows.length == 0) {
            grid.waPanel30.createTable();
        }
        else {
            if (tableRows.length < grid.pageRowCount) grid.waPanel30.updateTableRows();
        }
    ...
    setDataPanel1(param) {
        ...
        /* create table tbody */
        const tableRows = document.querySelectorAll(selector + ' .wa-grid-' + panelName + ' .wa-grid-table tbody tr');
        let tableRowIndex = 0;
    ...
    setDataPanel2(param) {
        ...
        //     document.querySelector(selector + ' .wa-grid-panel21 td div').textContent = rowCount;
        // }
    }
    ...
    setDataPanel0(param) {
        ...
        if (panelName == 'panel30') {
            ...
            document.querySelector(selector + ' .wa-grid-panel21 td div').textContent = String(rowCount);
        }
    }
    ...
    panel30_select(eventPanelName) {
        ...
        const mouseClickEvent = function (e) {
            ...
            else if (e.target.classList.contains('wa-grid-html-button')) {}
            else if (e.target.classList.contains('wa-grid-html-link')) {}
            else return;
        ...
        const mouseDownEvent = function (e) {
            ...
            else if (e.target.classList.contains('wa-grid-html-checkbox')) { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
            else if (e.target.classList.contains('wa-grid-html-button'))   { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
            else if (e.target.classList.contains('wa-grid-html-link'))     { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
        ...
        const mouseUpEvent = function (e) {
            ...
            let isInArea = grid.isInPanel(e, eventPanelName);
            if (isInArea) {
                //======================================================================================================
                // if (e.target.classList.contains('wa-grid-html-checkbox')) {
                //     let rowIndex = e.target.parentNode.dataset.rowIndex;
                ...
                //         grid.setValue(rowIndex, columnName, newValue);
                //     }
                //     setTimeout(() => grid.waPanel30.setDataPanel(grid.getFirstRowIndex()), 50);
                // }
                // else if (e.target.classList.contains('wa-grid-html-button')) {
                //     let rowIndex = e.target.parentNode.dataset.rowIndex;
                //     let columnIndex = e.target.parentNode.dataset.columnIndex;
        ...
        const selectCell = function(e) {
            let tableCell;
            if      (e.target.classList.contains('wa-grid-html-icon'))     { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
            // else if (e.target.classList.contains('wa-grid-html-checkbox')) { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
            // else if (e.target.classList.contains('wa-grid-html-button'))   { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
            // else if (e.target.classList.contains('wa-grid-html-link'))     { targetName = 'icon'; tableCell = e.target.closest('.wa-grid-cell'); }
            // else if (e.target.classList.contains('wa-grid-html-img'))      { targetName = 'img' ; tableCell = e.target.parentNode.parentNode; }
            ...
            if (tableCell.rowSpan != undefined && tableCell.rowSpan > 1) {
                let tableRows = document.querySelectorAll(selector + ' .wa-grid-panel31 .wa-grid-table tbody tr:not([style*="display: none"])');
        ...
        const selectCellMove = function(e) {
            ...
            if (grid.fixedColumnIndex != -1) {
                if (eventPanelName == 'panel32') {
                    let panel32 = document.querySelector(selector + ' .wa-grid-panel32');
                    ...
                    // if (lastX < panelRight32 && document.querySelector(selector + ' .wa-grid-horizontal-scroll-bar').style.left != '0px') {
                    //     flagLeft = true;
                ...
                let panel = document.querySelector(selector + ' .wa-grid-' + eventPanelName);
        ...
        const eventPanel = document.querySelector(selector + ' .wa-grid-' + eventPanelName);
    ...
    panel31_select(eventPanelName) {
        ...
        let eventPanel = document.querySelector(selector + ' .wa-grid-' + eventPanelName);

        const mouseClickEvent = function (e) {
            if (e.target.classList.contains('wa-grid-html-checkbox')) {} else return;
        ...
        const mouseDownEvent = function (e) {
            ...
            //console.log(`e.target.className ${e.target.className}`);
            let tableCell;
            if      (e.target.classList.contains('wa-grid-cell-div'))    { tableCell = e.target.parentNode; }
            else if (e.target.classList.contains('wa-grid-html-string')) { tableCell = e.target.parentNode.parentNode; }
            else if (e.target.classList.contains('wa-grid-cell'))        { tableCell = e.target; }
            else return;
