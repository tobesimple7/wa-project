...
export class WaGridPanel40 extends WaGridPanelBase {
    ...
    createHtml(parentElement) {
        ...
        let s = '';
        s += '<div class="wa-grid-group41">';
            s += '<div class="wa-grid-panel">';
            s += '<div class="wa-grid-panel41"><table class="wa-grid-table"></table></div>';
            s += '<div class="wa-grid-panel42"><table class="wa-grid-table"></table></div>';
            s += '</div>';
        s += '</div>';
        s += '<div class="wa-grid-group40">';
            s += '<div class="wa-grid-panel">';
            s += '<div class="wa-grid-panel40"><table class="wa-grid-table"></table></div>';
            s += '</div>';
        s += '</div>';
    ...
    panel40_select(eventPanelName) {
        ...
        const selectCellMove = function(e) {
            ...
            if (grid.fixedColumnIndex != -1) {
                let panel  = document.querySelector(selector + ' .wa-grid-panel32');
                ...
                let panel = document.querySelector(selector + ' .wa-grid-panel30'); // + eventPanelName);
                let rect = panel.getBoundingClientRect();
                let absRect = grid.getOffset(panel);
...
//console.log(`eventPanelName ${eventPanelName} 1) $maxRowIndex  ${maxRowIndex} maxCellIndex ${maxCellIndex}`);
    ...
    panel41_select(eventPanelName) {
        ...
        const mouseDownEvent = function (e) {
            ...
            //console.log(`e.target.className ${e.target.className}`);
            let tableCell;
            if      (e.target.classList.contains('wa-grid-cell-div'))      { targetName = 'div' ; tableCell = e.target.parentNode; }
            else if (e.target.classList.contains('wa-grid-cell'))          { targetName = 'cell'; tableCell = e.target; }
            else return;
        ...
        const selectCellMove = function(e) {
            ...
            let lastY = grid.lastY;
            if (grid.fixedColumnIndex != -1) {
                const panel  = document.querySelector(selector + ' .wa-grid-' + eventPanelName);
                const panel30= document.querySelector(selector + ' .wa-grid-panel30');
